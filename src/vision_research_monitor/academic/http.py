...
class AcademicHttpClient:
    ...
    def __enter__(self) -> AcademicHttpClient:
        return self
    ...
    def get_json(self, path: str, *, params: Mapping[str, Any] | None = None) -> HttpResult:
        ...
        try:
            ...
            raise AcademicApiError(f"Expected JSON response from {response.request.url}") from exc
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt >= self.max_retries:
                    raise AcademicApiError(f"Academic API transport failure: {exc}") from exc
            ...
            if response.status_code == 429 or 500 <= response.status_code < 600:
                if attempt >= self.max_retries:
                    raise AcademicApiError(
                        f"Academic API retry limit exceeded ({response.status_code}): {response.request.url}"
                    )
                delay = self._retry_delay(response, attempt)
                if delay > self.max_retry_wait:
                    raise AcademicApiError(
                        f"Academic API retry wait ({delay:.0f}s) exceeds configured maximum: {response.request.url}"
                    )
                self.sleeper(delay)
                continue
            ...
            try:
                ...
                raise AcademicApiError(
                    f"Academic API request failed ({response.status_code}): {response.request.url}"
                ) from exc
            return response
