...
from vision_research_monitor.sources.feeds import ResearchFeedCollector
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Collect relevant official research blog posts")
    ...
    state_store = JsonStateStore(ROOT / "data/state/research_blogs.json")
    state = state_store.load()

    if not config["research_blogs"]["enabled"]:
        print(json.dumps({"found": 0, "written": 0, "failed_targets": 0, "disabled": True}))
        return 0

    with HttpClient("https://example.invalid") as client:
        result = ResearchFeedCollector(client, state, config, taxonomy, classifier).collect(
            run_at,
            window_start=start,
            window_end=end,
        )
    ...
    if result.failed_targets == 0 and not explicit:
        state.setdefault("sources", {}).setdefault("research_blogs", {})["last_successful_at"] = to_iso8601(run_at)
